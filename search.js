...
    console.log("Ошибка запуска WebApp:", e);
}
...
                    } else {
                        alert('🤖 ИИ Gemini Pro не нашел подходящих фильмов. Попробуй другое описание!');
                    }
                })
                .catch(err => {
                    document.querySelector('input').value = query;
                    alert('Ошибка подключения к ИИ-серверу KINONEOX.');
                    console.error(err);
                });
        }
    }
...
               
    if (!grid) {
        alert('Ошибка: На сайте не найден неоновый блок для вывода фильмов.');
        return;
    }
    
...
                <img src="${posterUrl}" alt="${movie.title}" style="width: 100%; height: 100%; object-fit: cover; position: absolute; top:0; left:0; onerror="this.style.display='none'">
                🎬
...
                <div style="font-size: 14px; font-weight: 700; color: #ffffff; margin-bottom: 4px; letter-spacing: -0.1px;">${movie.title}</div>
                <div style="font-size: 11px; color: #6a6273; margin-bottom: 14px; font-weight: 600;">${movie.year} • ${movie.genre.toUpperCase()}</div>
                <button style="background: linear-gradient(180deg, #b51a34, #730f1e); color: #ffffff; border: none; padding: 11px; border-radius: 10px; font-size: 11px; font-weight: bold; cursor: pointer; text-transform: uppercase; box-shadow: 0 4px 15px rgba(181, 26, 52, 0.6); width: 100%; max-width: 200px; border-top: 1px solid #ff2a4b;" onclick="alert('🍿 Создаем приватную комнату для фильма: ${movie.title}...')">Создать</button>
            </div>
        `;
